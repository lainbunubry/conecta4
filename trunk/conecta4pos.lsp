...
(defstruct (nodo-j (:constructor crea-nodo-j)
	...
	posicion) 	;; Ultima posicion modificada	
...
(defun escribe-nodo-j (nodo-j &optional (canal t) profundidad)
	...
	(format canal "~%Último movimiento : ~a" (posicion nodo-j))
...
;; 		(list 'minimax-a-b '1) ;; algoritmo y profundidad
  ...
  (cond (empieza-la-maquina? (crea-nodo-j-inicial 'max)
                             (if (es-estado-final *estado-inicial* nil)
...
;;     (setf *ultimo-movimiento* (compara-tableros (estado nodo-j) (estado siguiente))) ;; Elección de la máquina
    (if (es-estado-final (estado siguiente) (posicion nodo-j))
    ...
    (let ((m (read)))
	 (cond ((equal m 'consejo)				;; En el caso de que quiera pedir consejo
		...
		(let ((nuevo-estado
			...
			(pos (list (primera-posicion-vacia (estado nodo-j) m) m)))
			(cond (nuevo-estado
...
;; 				(setf *ultimo-movimiento* (compara-tableros (estado nodo-j) (estado siguiente))) ;;Elección del humano
	                        (if (es-estado-final nuevo-estado (posicion nodo-j))
     	                       		(analiza-final siguiente)
...
(defun aplica-decision (procedimiento nodo-j)
	(format t "aplica-decision ~a ~a"procedimiento nodo-j) ;;debug
...
(defun aplica-movimiento (columna tablero color)
	(let ((fila (primera-posicion-vacia tablero columna))
		...
		((null fila)
;; 			nil)
		(format t "aplica movimiento fila nil"))
			(t
...
;; TODO añadirle a estado final la posicion
...
(defun es-estado-final (tablero posicion)
(cond ((null posicion) ;;Primera iteracion
(format t "es estado final posicion nil") ;;debug
nil)
	(t
	(or
		(<= (length (movimientos-legales tablero)) 0)
...
(defun sucesores (nodo-j)
  (let ((resultado ()))
    (loop for movimiento in *movimientos* do
      (let ((siguiente
             	(aplica-movimiento movimiento (estado nodo-j) *color-maquina*))
		(fila (primera-posicion-vacia (estado nodo-j) movimiento)))
;; 	(format t "sucesores fila :~a" fila) ;;debug
	(when siguiente
        	(push
            	(crea-nodo-j
			:estado siguiente
			:jugador (contrario (jugador nodo-j))
			:posicion (list fila movimiento))
            resultado))))
    (nreverse resultado)))
...
(defun minimax (nodo-j profundidad)
  (if (or (es-estado-final (estado nodo-j) (posicion nodo-j))
          (= profundidad 0))
      (crea-nodo-j :valor (f-e-estatica (estado nodo-j)
                                        (jugador nodo-j)
					(posicion nodo-j)))
...
(format t "maximizador a-b ~a" profundidad) ;;debug
  (let ((mejor-sucesor (first sucesores))
        (valor 0))
    (loop for sucesor in sucesores do
          (setf valor
...
(format t "minimizador a-b ~a" profundidad) ;;debug
  (let ((mejor-sucesor (first sucesores))
        (valor 0))
    (loop for sucesor in sucesores do
          (setf valor
...
(defun f-e-estatica (tablero jugador posicion)
	(cond
		((null posicion) 
			(format t "posicion nula")  ;;Debug
		)
		((equal *jugador-maquina* jugador)
			(imprime-tablero tablero) ;;debug
			(format t "Posicion: ~a ,Heuristica maquina f-e-estatica ~a ~%"posicion (heuristica-4 tablero posicion *color-maquina*)) ;; debug
			(heuristica-4 tablero posicion *color-maquina*))
		(t
			(imprime-tablero tablero) ;;debug
			(format t "Posicion: ~a ,Heuristica humano f-e-estatica ~a ~%"posicion (heuristica-4 tablero posicion *color-humano*)) ;; debug
...
(defun compara_heurs (heuristica1 heuristica2)
	...
	(es-estado-final *estado-inicial* (posicion nodo-j-inicial))
	(analiza-final *nodo-j-inicial*)
